...
class Message(SQLModel, table=True):
    ...
    role: str = Field(min_length=1, nullable=False)
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), nullable=False)
    
    session_id: int = Field(foreign_key="user_sessions.id", nullable=False)
...
class QuizItem(SQLModel, table=True):
    ...
    session: UserSession = Relationship(back_populates="quiz_items")
...
class EvaluationResult(SQLModel, table=True):
    ...
    submitted_answer_id: int = Field(foreign_key="submitted_answers.id", nullable=False)
